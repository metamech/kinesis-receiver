...
API.prototype.createPort = function(port) {
    if (!port || !port.port) {
        console.error('Invalid port specification')
    ...
    self.server.listen(null, host, function(err) {
        if (err && err.code == 'EADDRINUSE') {
            ...
            setTimeout(function() {
                ...
                self.server.listen(null, host, function(err) {
                    if (err) {
                        console.error('Could not start API %s', host)
                        done(err)
                    } else {
                        var addr = self.server.address()
                        self.conf.api = {host: addr.address, port: addr.port}
                        console.log('%s api listening at %s:%d', self.conf.name, self.conf.api.host, self.conf.api.port)
                        done(null, self.conf)
                    }
                })
